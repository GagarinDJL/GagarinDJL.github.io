...
- 新建通用自定义Block：默认使用 `-custom` 后缀（如 `collection-custom`）
- 首页、详情页、导航类Block：可沿用既有语义命名（如 `projects-home`、`awards-detailed`、`back-to-home`）
...
**主题Block位置：**
```bash
find ~/Library/Caches/hugo_cache/modules/filecache/modules/pkg/mod \
...
cp "$THEME_BLOX_DIR/resume-biography-3/block.html" \
   layouts/partials/hbx/blocks/your-block-custom/block.html
```

说明：Hugo module cache 路径会随主题版本变化，复制模板前先用 `find` 获取当前路径。
...
```
find ~/Library/Caches/hugo_cache/modules/filecache/modules/pkg/mod \
  -path '*/github.com/!hugo!blox/hugo-blox-builder/modules/blox-tailwind@*/data/icons' \
  -type d
```
...
本项目同时保留了 `collection-custom`：普通内容列表使用主题自带 `collection`；需要论文自定义排序时使用 `collection-custom` 并设置 `content.custom_sort: true`。
...
如果需要论文自定义排序，使用 `collection-custom` 并在 `content` 中设置 `custom_sort: true`：
...
排序规则：先显示带 `weight` 的出版物（`weight` 升序），再按发表状态、一作身份、日期排序；draft 出版物会被过滤。
...
| `collection-custom` | 论文/内容集合，支持自定义排序 | `layouts/partials/hbx/blocks/collection-custom/block.html` | 文件夹中的 markdown 文件 |
...
| `awards-custom` | 奖项展示通用自定义版本 | `layouts/partials/hbx/blocks/awards-custom/block.html` | `content/authors/admin/_index.md` |
...
| `resume-skills-custom` | 技能展示 | `layouts/partials/hbx/blocks/resume-skills-custom/block.html` | `content/authors/admin/_index.md` |
...
**命名规则：通用自定义Block默认添加 `-custom` 后缀；首页、详情页、导航类Block可使用现有语义命名模式。**
...
5. **清晰命名** - 通用自定义Block使用 `-custom`，页面语义Block沿用 `*-home`、`*-detailed` 等模式
6. **深色模式** - 始终定义 `dark:` 变体
